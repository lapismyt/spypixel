...
def generate_svg_badge(
    text_left: str, text_right: str, color_right: str = "#4c1"
) -> str:
    ...
    left_content_width_px = (
        len(text_left_safe) * CHAR_WIDTH_PX + PADDING_PER_SIDE_PX * 2
    )
    right_content_width_px = (
        len(text_right_safe) * CHAR_WIDTH_PX + PADDING_PER_SIDE_PX * 2
    )
    ...
    left_text_length = max(
        10, round(left_segment_width_px * 10 - TEXT_PADDING_FOR_TEXTLENGTH * 2)
    )
    right_text_length = max(
        10, round(right_segment_width_px * 10 - TEXT_PADDING_FOR_TEXTLENGTH * 2)
    )
